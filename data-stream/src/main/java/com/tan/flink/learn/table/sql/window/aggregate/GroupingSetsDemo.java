 ...
 * create time: 2022/11/22 13:46
 * describe content: flink-1.16.0-learn
 */
public class GroupingSetsDemo {
    ...
    public static void main(String[] args) {
        ...
        tableEnv.sqlQuery("SELECT `window_start`, `window_end`, `user`, count(1) as cnt\n" +
                        "  FROM TABLE(\n" +
                        "    TUMBLE(TABLE socket_info, DESCRIPTOR(ts_ltz), INTERVAL '5' SECONDS))\n" +
                        "  GROUP BY window_start, window_end, GROUPING SETS ((`user`),())")
                .execute()
                .print();
